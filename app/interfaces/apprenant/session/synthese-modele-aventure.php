...
<body>
	...
	<script type="text/javascript">
		...
		$( document ).ready( function () {
			...
			selection = getLocalData( '0x6176656e74' );
			...
			$.each( selection, function( index, val ) {
				...
				_html += '<div class="col-3"><div class="card"><img src="http://commun.alteretgo.my-workflow.fr/aventure/' + _srcImg[ val.img ] + '.png"><span class="wordSelected">';
				if ( _correct[ val.img ] == val.word ) {
					_html += '<span class="answered green-text">' + _srcWrd[ val.word ] + '</span>';
				}
				else{
					_html += '<span class="answered orange-text">' + _srcWrd[ val.word ] + '</span><span class="correctif green-text">' + _srcWrd[ _correct[ val.img ] ] + '</span>';
				}
				_html += '</span></div></div>';
				if ( i == j-1 ) {
					_html += '</div>';
			...
			if ( parseFloat( selection.score ) == 1 ) {
				...
				$( '#comment' ).html( 'Félicitations, vous savez retranscrire le modèle de l’aventure en contexte professionnel.' ).addClass( 'visible' );
				notif( 'valid', '<i class="material-icons left">star</i> Félicitations, vous savez retranscrire le modèle de l’aventure en contexte professionnel.<br>Vous gagnez une étoile.', 8 );
				if( typeof _stars[ '0x6176656e74' ] === "undefined" ) { addStar( '0x6176656e74' ); }
			}
		} );
	</script>
</body>
