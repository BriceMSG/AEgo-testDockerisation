...
<body>
	...
	<div id="content" class="">
		<div class="card w768 block center quizzFinal">
			<div class="elem">
	...
	<script type="text/javascript">
		...
		function callbackMsg( _msg ) {
			...
			if ( _msg.msg == 'goto' ) {
				if ( _msg.data == 'certificat.php' ) {
					setLocalData( 'pathname', '/session/certificat.php' );
					window.location = _uri + 'certificat.php?_session=' + _ssid + '&uuid=' + _uuid;
				}
				else {
					goTo( _msg.data );
				}
		...
		function retriveAnswer( _answers ) {
			...
			_score = ( _score / _nbr ).toFixed( 2 );

			if ( parseFloat( _score ) >= 0.75 ) {
				_stars = getLocalData( 'stars' );
				$( '#star' ).html( '<i class="material-icons">star</i>' );
				$( '#comment' ).html( 'Félicitations, vous avez bien intégré les concepts de la conduite du changement.' ).addClass( 'visible' );
				notif( 'valid', '<i class="material-icons left">star</i> Félicitations, vous avez bien intégré les concepts de la conduite du changement.<br>Vous gagnez une étoile.', 8 );
				if( typeof _stars[ '0x66696e616c' ] === "undefined" ) { addStar( '0x66696e616c' ); }
			}
		};
		...
		$( document ).ready( function () {
			...
			selection = getLocalData( '0x66696e616c' );

			recoverySave( 'getQuizzGrp.php', { "ssid":getLocalData( 'ssid' ), "quizz":'0x66696e616c', "uuid":_uuid }, retriveAnswer );
		} );
	</script>
</body>
</html>
