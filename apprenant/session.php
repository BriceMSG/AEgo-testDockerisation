<?php
	...
	if( $return === false ) {
		...
		<div class="row">
			<div class="col-2"></div>
			<div class="col-8">
				<a href="http://apprenant.alteretgo.my-workflow.fr/session.php?formateur=' . $formateur . '" class="btn alter-btn white-text txt-center block center">
		...
		if( isset( $_GET[ 'uuid' ] ) === false ) {
			# Jonction d'une session, création des identifiants
			$uuid = uniqid();

			# Preparation requet
			$sqlQuery = 'INSERT INTO {pre_}apprenant ( ssid, uuid ) VALUES ( ?, ? )';
			$sqlParam = [ $ssid, $uuid ];
		...
		<div class="valign">
			<div class="card w512 center block">
				...
				<div id="bug">
					...
					<a href="http://apprenant.alteretgo.my-workflow.fr/session.php?formateur=<?php echo $formateur; ?>&amp;uuid=<?php echo $uuid; ?>" class="btn alter-btn white-text txt-center center block">
...
<?php if( $return !== false ) { ?>
	<script type="text/javascript">
		function goto(){
			window.location = "http://apprenant.alteretgo.my-workflow.fr/session/connexion.php?_session=<?php echo $ssid; ?>";
		};
		function hasStorage() {
			try {
				if ( localStorage.getItem( 'ssid' ) === null ) { localStorage.setItem( 'ssid', '<?php echo $ssid; ?>' ); }
				if ( localStorage.getItem( 'uuid' ) === null ) { localStorage.setItem( 'uuid', '<?php echo $uuid; ?>' ); }
				if ( localStorage.getItem( 'instructor' ) === null ) { localStorage.setItem( 'instructor', '<?php echo $formateur; ?>' ); }
				if ( localStorage.getItem( 'stars' ) === null ) { localStorage.setItem( 'stars', '{"nbr":0}' ); }
				if ( localStorage.getItem( 'pathname' ) === null ) { localStorage.setItem( 'pathname', '/session/connexion.php' ); }
